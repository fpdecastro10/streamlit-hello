...
def main():
    with st.sidebar:
        ...
        # Filtrer new clients or clients with history
        type_of_simulation = st.selectbox("Tipo de cliente", ["Cliente Nuevo", "Cliente con historial de campaña"])

        # Filter name of campaign
        opciones_seleccionadas = st.selectbox("Filtre por nombre de campaña:", unique_combination_store_group)

        # Dataset with the name of store group in this campaign
        unique_combinationsStore = unique_combinations.query(f"campaign in @opciones_seleccionadas")

        # Instance of the dict { store_group_id-name_store_group : 
        index_storeGroup = {}
